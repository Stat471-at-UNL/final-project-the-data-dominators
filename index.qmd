...
We developed an R package with cleaned 2023 and 2024 Missouri BRFSS
...
For the Tobacco Use topic, we observe 4 different variables pertaining
to tobacco use. There are 14,528 rows of data containing observations.
...
The BRFSS data comes with lots of missing data, as well an overwhelming structure. In response, the cleaning process was naturally split up into a 3-step process: investigating the missing values, imputing the missing values, and tidying the dataset.

### Investigating Missing Values

We first examine the extent of missing data for the tobacco, cognitive,
...
Several items had a large amount of missing data, especially `CDHOUS1`
and `CDSOCIA1` with over 40–90% missing or NA, so we excluded them from
the imputation models.

We then tested whether the data were Missing Completely At Random (MCAR)
using Little’s MCAR test (`naniar::mcar_test`), applied separately to
tobacco and cognitive variables in each year. Below is an example of the
...
In all cases, Little’s MCAR test was highly significant(p \<
`r mcar_tobacco_2023$p.value`), providing strong evidence against MCAR
and supporting the use of multiple imputation.
...
the output revealed no duplicate SEQNO ID/numbers. This documents it as
...
As an example analysis, this code looks at the average `SMOKE100` value among different ages, with the averages using the different imputations overlay-ed.
...
# Summary plot
smoke_by_age %>%
  ggplot(aes(x = AGE80, y = AVG_SMOKE100)) + geom_bar(stat = "identity") + geom_point(aes(y = AVG_SMOKE100_IMP1, color = "Imputation 1")) + geom_point(aes(y = AVG_SMOKE100_IMP2, color = "Imputation 2")) + geom_point(aes(y = AVG_SMOKE100_IMP3, color = "Imputation 3"))
```
...
the same trend, providing additional confirmation in the chosen imputation method.
